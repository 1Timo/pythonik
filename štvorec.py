...
class Bar:
    def __init__(self, starting: list = None, active=False, categ: tuple = (), working: Button = None, error=()):
        ...
        self.frame.bind("<Configure>", lambda event: self.canvas.configure(
            scrollregion=self.canvas.bbox("all"), width=width / 4 + 15, height=height // 2))
...
class Goods:
    def __init__(self, code: str, name: str, sell: str, buy: str, row: int, color1: str, color2: str, frame: Frame):
        ...
        self.label = Label(self.frame, text=self.code + ':', relief=RAISED, bd=1, width=15, bg=color1)
        self.label.grid(row=self.row, column=0)
        self.label2 = Label(self.frame, anchor=W, text=self.name, relief=RAISED, bd=1, width=15, bg=color1)
        self.label2.grid(row=self.row, column=1)
        self.entry1 = Entry(self.frame, textvariable=self.sell_price, width=20, bg=color2)
        self.entry1.grid(row=self.row, column=2)
        self.entry2 = Entry(self.frame, textvariable=self.buy_price, width=20, bg=color2)
